...
export const Vortex = (props: VortexProps) => {
  ...
  const drawParticle = (
    ...
    // Use the official Solana brand colors
    let color
    const canvasHeight = ctx.canvas.height
    const yPosition = y / canvasHeight

    if (yPosition < 0.33) {
      // Top section - Solana Green ("Surge Green")
      color = `hsla(157, 88%, 51%, ${fadeInOut(life, ttl)})` // #14F195
    } else if (yPosition < 0.66) {
      // Middle section - Solana Purple ("Purple Dino")
      color = `hsla(270, 100%, 64%, ${fadeInOut(life, ttl)})` // #9945FF
    } else {
      // Bottom section - Solana Blue ("Ocean Blue")
      color = `hsla(190, 100%, 50%, ${fadeInOut(life, ttl)})` // #00C9FF
    }
  ...
  return (
    <div className={cn("relative w-full", props.containerClassName)}>
      <div
        ref={containerRef}
        className={cn(
          "fixed h-full w-full inset-0 z-0 bg-transparent flex items-center justify-center transition-opacity duration-1000",
